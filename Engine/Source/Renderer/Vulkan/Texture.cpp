...
namespace oe::Renderer::Vulkan
{
    ...
    {
        mData.Pixels = stbi_load(path.c_str(), &mData.Width, &mData.Height, &mData.Channels, STBI_rgb_alpha);
    ...
    void Texture::Load()
    {
        ...
        const uint32_t mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(mData.Width, mData.Height)))) + 1;
        ...
        Image::Create(mData.Width, mData.Height, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mImage, mDeviceMemory, VK_SAMPLE_COUNT_1_BIT, mipLevels);

        Image::TransitionLayout(mImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
        Buffer::CopyToImage(stagingBuffer, mImage, static_cast<uint32_t>(mData.Width), static_cast<uint32_t>(mData.Height));

        vkDestroyBuffer(device, stagingBuffer, nullptr);
        vkFreeMemory(device, stagingBufferMemory, nullptr);

        Image::CreateView(&mImageView, mImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
    ...
    void Image::TransitionLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipmaps)
    {
    ...
    void Image::Create(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, VkSampleCountFlagBits numSamples, uint32_t mipmaps)
    {
        const auto device = GetLogicalDevice()->Get();
    ...
    void Image::CreateViewInfo(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipmaps, VkImageViewCreateInfo& viewInfo)
    {
    ...
    void Image::CreateView(VkImageView* imageView, VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipmaps)
    {
        VkImageViewCreateInfo viewInfo{};
        CreateViewInfo(image, format, aspectFlags, mipmaps, viewInfo);
        VK_CHECK_RESULT(vkCreateImageView(GetLogicalDevice()->Get(), &viewInfo, nullptr, imageView), "Failed to create texture image view!")
    }
    ...
    void Texture::CreateTextureSampler(VkSampler* sampler, uint32_t mipmaps)
    {
        ...
        VK_CHECK_RESULT(vkCreateSampler(GetLogicalDevice()->Get(), &samplerInfo, nullptr, sampler), "Failed to create texture sampler!")
    }
