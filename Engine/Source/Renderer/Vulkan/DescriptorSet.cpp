...
namespace oe::Renderer::Vulkan
{
	void DescriptorSet::Begin(VkDescriptorPool pool, const VkDescriptorSetLayout* layout)
	{
		...
		VK_CHECK_RESULT(vkAllocateDescriptorSets(GetLogicalDevice()->Get(), &allocInfo,
			                &mDescriptorSet), "Failed to allocate descriptor sets!")
	}
	...
	void DescriptorSet::AddBufferWriteDescriptor(int binding, VkBuffer buffer, VkDeviceSize bufferSize)
	{
		...
		vkUpdateDescriptorSets(GetLogicalDevice()->Get(), mDescriptorWrites.size(), mDescriptorWrites.data(), 0,
		                       nullptr);
	}
