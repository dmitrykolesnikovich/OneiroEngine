...
namespace oe::Renderer::Vulkan
{
	void DepthBuffer::Create()
	{
		...
		Texture::CreateImage(GetSwapChain()->GetExtent2D().width, GetSwapChain()->GetExtent2D().height, depthFormat,
		                     VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
		                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mImage, mImageMemory,
		                     GetPhysDevice()->GetMsaaSamples());
		Texture::CreateImageView(&mImageView, mImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
	}
	...
	VkImageView DepthBuffer::GetView() const { return mImageView; }
